...
def prepare_mesh_fast(obj):
    """
    Triangulate and refresh normals ahead of FBX export. This previously
    caught ANY exception and returned False, and the only call site used
    that return value to filter the export selection -- so a single failing
    call here (e.g. free_normals_split()/calc_normals_split() being removed
    in Blender 4.1+'s mesh API) silently dropped the object from the FBX
    export entirely, with no error surfaced anywhere. Now: normal-related
    calls are version-guarded and non-fatal, and only a genuine bmesh
    failure is logged loudly -- but no failure here removes the object from
    export, since a mesh with imperfect normals is still far better than a
    mesh that's silently missing.
    """
    mesh = obj.data

    if hasattr(mesh, "has_custom_normals") and mesh.has_custom_normals and hasattr(mesh, "free_normals_split"):
        try:
            mesh.free_normals_split()
        except Exception as e:
            print_to_stream(f"WARNING: free_normals_split failed on '{obj.name}': {e}", stream=None, base_time=time.time())
    ...
    try:
        ...
        print_to_stream(f"ERROR: Triangulation failed on '{obj.name}', exporting as-is: {e}", stream=None, base_time=time.time())

    if hasattr(mesh, "calc_normals_split"):
        try:
            mesh.calc_normals_split()
        except Exception as e:
            print_to_stream(f"WARNING: calc_normals_split failed on '{obj.name}': {e}", stream=None, base_time=time.time())
...
def compute_wall_groups(mesh, roof_threshold):
    """
    Cluster non-roof polygons into adjacent wall groups (faces that share an
    edge and point in roughly the same direction). Computed ONCE per building
    and reused by both batch_create_doors and batch_create_windows, instead of
    each function recomputing the same O(n^2) adjacency search independently.
    """
    wall_groups = []
    used_polys = set()
    ...
    for poly in mesh.polygons:
        if poly.index in used_polys:
            continue
        if abs(poly.normal.z) > roof_threshold:
            continue  # skip roofs
        ...
        for other_poly in mesh.polygons:
            if other_poly.index in used_polys:
                continue
            if abs(other_poly.normal.z) > roof_threshold:
                continue

            # Similar direction (~30 degrees) AND sharing an edge, so we don't
            # merge disconnected faces that just happen to face the same way.
...
def random_door_gray_hex(min_hex="#121212", max_hex="#333333"):
    """Pick a random neutral gray hex value between two grayscale hex bounds."""
    lo = int(min_hex.lstrip('#')[0:2], 16)
    hi = int(max_hex.lstrip('#')[0:2], 16)
    val = random.randint(lo, hi)
    return f"#{val:02x}{val:02x}{val:02x}"


def get_or_create_door_material(texture_folder, export_folder):
    """
    Try to grab a random texture from a '<texture_folder>/doors' folder (PBR
    set or single image, reusing the same loader as walls/roofs). Falls back
    to a flat, randomly varied dark-gray material if no doors folder/textures
    exist yet, so doors still get color variation today and 'just work' once
    real door textures are dropped in later.
    """
    doors_texture_folder = os.path.join(texture_folder, "doors") if texture_folder else None

    if doors_texture_folder and os.path.exists(doors_texture_folder):
        mat = get_or_create_material_auto("", doors_texture_folder, export_folder, random_select=True)
        if mat:
            return mat

    # Fallback: flat random dark-gray material, cached per exact shade so we
    # don't create a new material datablock for every single building.
    hex_color = random_door_gray_hex()
    mat_name = f"Door_Flat_{hex_color.lstrip('#')}"
    mat = bpy.data.materials.get(mat_name)
    if mat:
        return mat
    ...
    r = srgb_to_linear(int(hex_clean[0:2], 16))
    g = srgb_to_linear(int(hex_clean[2:4], 16))
    b = srgb_to_linear(int(hex_clean[4:6], 16))
...
def batch_create_doors(building_obj, settings, doors_collection, wall_groups=None):
    """
    Fast creation of doors for a building using a single mesh per building.
    Returns (door_obj, doors_meta) where doors_meta is a list of dicts with
    the real width/height/center/normal of each placed door, so that data
    doesn't get thrown away at export time.

    wall_groups: pre-computed groups from compute_wall_groups(), shared with
    batch_create_windows so the adjacency search only runs once per building.
    If not supplied, it's computed here (kept for backwards compatibility).
    """
    ...
    if building_obj.type != 'MESH' or not building_obj.data:
        return None, doors_meta
    
    mesh = building_obj.data
    
    # Safety check - ensure mesh has vertices
    if len(mesh.vertices) == 0:
        return None, doors_meta  # Silently skip empty meshes
    ...
    # Reuse the shared wall-group computation, then keep only the groups that
    # actually touch the ground (doors don't belong on upper floors).
    if wall_groups is None:
        wall_groups = compute_wall_groups(mesh, settings.roof_threshold)

    ground_wall_groups = []
    for group in wall_groups:
        touches_ground = any(
            min(mesh.vertices[vi].co.z for vi in mesh.polygons[pidx].vertices) <= ground_z + 0.1
            for pidx in group
        )
        if touches_ground:
            ground_wall_groups.append(group)

    # PLACE ONE DOOR PER WALL GROUP
    doors_placed = 0
    for wall_group in ground_wall_groups:
        ...
        # Store real metadata (this is what actually reaches Unity now,
        # instead of being written to unused custom properties)
        door_global_id += 1
        door_center_local = door_position + Vector((0, 0, door_h / 2))
        doors_meta.append({
            "id": door_global_id,
            "width": round(door_w, 3),
            "height": round(door_h, 3),
            "center": building_obj.matrix_world @ door_center_local,
            "normal": (building_obj.matrix_world.to_3x3() @ normal_local).normalized(),
        })
    ...
    # Assign material: random dark-gray shade (#121212-#333333), or a random
    # texture from <texture_folder>/doors if one exists.
    texture_folder = getattr(settings, "texture_folder", None)
    export_folder = getattr(settings, "export_folder", None)
    door_mat = get_or_create_door_material(texture_folder, export_folder)
    ...
    return door_obj, doors_meta

def batch_create_windows(building_obj, settings, windows_collection, wall_groups=None):
    """
    Fast creation of windows for a building using a single mesh per building.
    Returns (window_obj, windows_meta) where windows_meta is a list of dicts
    with the real width/height/world-center/world-normal of each window.

    wall_groups: pre-computed groups from compute_wall_groups(), shared with
    batch_create_doors so the adjacency search only runs once per building.
    Note: previously this function grouped walls by normal similarity alone
    (no shared-edge check), which could merge disconnected faces that happen
    to face the same direction on non-box buildings. Using the shared,
    edge-aware grouping fixes that as a side effect.
    """
    ...
    # Reuse the shared wall-group computation instead of recomputing it here
    if wall_groups is None:
        wall_groups = compute_wall_groups(mesh, settings.roof_threshold)
        ...
        for poly_idx in wall_poly_indices:
            ...
            for floor_idx in range(num_floors_for_face):
                ...
                for win_idx in range(num_windows_horizontal):
                    ...
                    windows_meta.append({
                        "id": len(windows_meta) + 1,
                        "width": round(settings.window_width, 3),
                        "height": round(settings.window_height, 3),
                        "center": window_center.copy(),
                        "normal": wall_normal.copy(),
                    })
    ...
    base_color = hex_to_linear_rgba("#212121")      # dark window glass
    glow_color = hex_to_linear_rgba("#4a90d9")       # soft blue glow, adjust as needed
    ...
    if window_mat_name not in bpy.data.materials:
        ...
        node_bsdf = window_mat.node_tree.nodes.get("Principled BSDF")

    # Set base color
    node_bsdf.inputs['Base Color'].default_value = base_color

    # Set glow (emission) — handle both old and new Blender input names
    emission_input = None
    if 'Emission Color' in node_bsdf.inputs:
        emission_input = node_bsdf.inputs['Emission Color']
    elif 'Emission' in node_bsdf.inputs:
        emission_input = node_bsdf.inputs['Emission']

    if emission_input:
        emission_input.default_value = glow_color

    try:
        node_bsdf.inputs['Emission Strength'].default_value = 2.0  # increase for stronger glow
    except KeyError:
        pass
    ...
    return window_obj, windows_meta
...
class WINDOWS_AND_DOORS(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        door_settings.texture_folder = bpy.path.abspath(scn.texture_folder) if scn.texture_folder else None
        door_settings.export_folder = bpy.path.abspath(scn.export_folder) if scn.export_folder else None
        ...
        for i, obj in enumerate(meshes, start=1):
            # Compute wall groups ONCE per building and share between doors
            # and windows, instead of each recomputing the same O(n^2)
            # adjacency search independently.
            shared_wall_groups = None
            if obj.type == 'MESH' and obj.data and len(obj.data.vertices) > 0:
                shared_wall_groups = compute_wall_groups(obj.data, door_settings.roof_threshold)

            # Create doors and windows
            door_obj, doors_meta = batch_create_doors(obj, door_settings, doors_collection, wall_groups=shared_wall_groups)
            window_obj, windows_meta = batch_create_windows(obj, window_settings, windows_collection, wall_groups=shared_wall_groups)
            console_progress(i, totals, start_time=start_time)
            GLOB_BUILDINGS_META_DATA.append((obj, window_obj, door_obj, windows_meta, doors_meta))
...
def export_city_metadata(buildings_data, settings):
    ...
    if buildings_data and isinstance(buildings_data[0], dict):
        ...
        for entry in buildings_data:
            # New format carries the real metadata collected while placing
            # each door/window (width/height/center/normal). Old 3-tuple
            # format is still supported as a fallback, but note it previously
            # produced one JSON entry per cube FACE for doors (6 per door,
            # mostly bogus) and never included width/height at all.
            if len(entry) == 5:
                building_obj, window_obj, door_obj, windows_meta, doors_meta = entry
            else:
                building_obj, window_obj, door_obj = entry
                windows_meta, doors_meta = None, None
            ...
            if windows_meta is not None:
                for w in windows_meta:
                    building_data["windows"].append({
                        ...
                        "width": w["width"],
                        "height": w["height"],
                        "center": round_vec(blender_to_unity_coords([w["center"].x, w["center"].y, w["center"].z]), 3),
                        "normal": round_vec(blender_to_unity_coords([w["normal"].x, w["normal"].y, w["normal"].z]), 3)
                    })
            elif window_obj and window_obj.data:
                # Fallback path (old format): derive from mesh polygons
                mesh = window_obj.data
                for poly in mesh.polygons:
                    center = sum((window_obj.matrix_world @ mesh.vertices[v].co for v in poly.vertices), Vector()) / len(poly.vertices)
                    building_data["windows"].append({
            ...
            if doors_meta is not None:
                for d in doors_meta:
                    building_data["doors"].append({
                        ...
                        "width": d["width"],
                        "height": d["height"],
                        "center": round_vec(blender_to_unity_coords([d["center"].x, d["center"].y, d["center"].z]), 3),
                        "normal": round_vec(blender_to_unity_coords([d["normal"].x, d["normal"].y, d["normal"].z]), 3)
                    })
            elif door_obj and door_obj.data:
                # Fallback path (old format): derive from mesh polygons
                mesh = door_obj.data
                for poly in mesh.polygons:
                    center = sum((door_obj.matrix_world @ mesh.vertices[v].co for v in poly.vertices), Vector()) / len(poly.vertices)
...
class EXPORT_DATA(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # Pack images — skip any missing on disk individually instead of
        # calling pack_all() on everything, which tries the missing ones too
        # and dumps noisy per-file errors even though it does continue.
        ...
        for img in bpy.data.images:
            # Already packed (e.g. embedded in the .blend) - nothing to do.
            ...
            try:
                ...
                print_to_stream(f"WARNING: Failed to pack '{img.name}', skipping: {e}", stream=None, base_time=time.time())
                missing_images.append(img.name)
                skipped_count += 1

        if missing_images:
            print_to_stream(
                f"WARNING: {len(missing_images)} image(s) missing on disk, skipped (export continues): {missing_images}",
                stream=None, base_time=time.time()
            )

        print_to_stream(f"Packed {packed_count} image(s), skipped {skipped_count} missing image(s).", stream=None, base_time=time.time())
        ...
        # Filter meshes properly. NOTE: membership here is by type only --
        # prepare_mesh_fast() below is best-effort cleanup and must never be
        # allowed to remove a building from the export (see its docstring).
        objects_to_export = [obj for obj in all_objects if obj.type == 'MESH']
